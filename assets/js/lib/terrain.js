...
Terrain.prototype.placePlayers = function() {
  ...
  _.forEach(positionAttaque, function(pos, poste) {
    this.getItemByName("t1" + poste).position = pos;
  }, this);
};
...
Terrain.prototype.placeDefence = function(defenseType) {
  ...
  _.forEach(positionDefense, function(pos, poste) {
    this.getItemByName("t2" + poste).position = pos;
  }, this);
};
...
function renderTerrain(x, y, longueur, largeur) {
  ...
  rect = new Paper.Rectangle(new Paper.Point(0, 0), new Paper.Point(offsetLeft + longueur + 10, offsetTop));
